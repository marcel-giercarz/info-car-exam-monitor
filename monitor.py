...
coloredlogs.install(level='DEBUG', logger=logging, fmt="[%(asctime)s] %(message)s", milliseconds=True, field_styles={ 'hostname': {'color': 'magenta'}, 'levelname': {'bold': True, 'color': 'black'}, 'name': {'color': 'blue'}, 'programname': {'color': 'cyan'}, 'username': {'color': 'yellow'}})
    ...
    def load_config(self):
        try:
            ...
            with open(f"{self.config_file}", "w") as f:
                json.dump({"login" : "","password" : "","max_exam_time" : 7,"category" : "B","word_id" : "","discord_webhook_url" : "","delay" : 10}, f, indent=4)
            exit()
        ...
        while retries < max_retries:
            try:
                with requests.Session() as s:
                    ...
                    try:
                        r = s.post(url, headers=headers, data=data, cookies=cookies, allow_redirects=True, verify=False, timeout=30)
                        cookies = s.cookies.get_dict()
                    except Exception as e:
                    ...
                    try:
                        r = s.get(
                            ...
                            headers=headers, cookies=cookies, allow_redirects=False, verify=False, timeout=30)
                ...
                if response.status_code == 401:
                    ...
                    for exam in response_json["schedule"]["scheduledDays"]:
                        if len(exam["scheduledHours"][0]["practiceExams"]) > 0:
                            ...
                            if datetime.strptime(exam_day + " " + exam_time, "%Y-%m-%d %H:%M:%S").timestamp() < max_date and datetime.strptime(exam_day + " " + exam_time, "%Y-%m-%d %H:%M:%S").timestamp() != last_date:
